...
export function ScrollProgress({
  ...
  useState(() => {
    if (typeof window !== 'undefined') {
      ...
      const activeBreakpoints =
        initialIsMobile && mobileBreakpoints ? mobileBreakpoints : breakpoints;
  ...
  useEffect(() => {
    ...
    const activeBreakpoints = [
      ...(isMobile && mobileBreakpoints ? mobileBreakpoints : breakpoints)
    ...
    const handleScroll = () => {
      ...
      if (nextBreakpoint) {
        ...
        const progress = Math.min(
          1,
          Math.max(0, (currentPercentage - currentBreakpoint.percentage) / range)
        );
  ...
  return (
    <div className="fixed right-4 top-24 z-50 hidden h-10 w-12 items-center justify-center overflow-hidden rounded-full bg-text font-mono text-sm text-background transition-colors duration-200">
